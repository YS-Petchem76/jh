...
def generate_questions_by_year(year):
    ...
    # 2014-2019년도 문제: 기본 보조 문제 셋
    for y in [2019, 2018, 2017, 2016, 2015, 2014]:
        questions[y] = [
            {"id": 100+i, "exam_type": "WRITTEN", "qtype": "MCQ", "subject": "냉동기계일반" if i < 30 else "공기조화", "prompt": f"【{y}년】냉동시스템 운전 관련 문제 {i+1}번", "choices": ["선택지 A", "선택지 B", "선택지 C", "선택지 D"], "answer": i % 4, "explanation": f"{y}년 기출문제 해설", "choice_explanations": ["❌", "❌", "✅", "❌"], "year": y}
            for i in range(60)
        ]
